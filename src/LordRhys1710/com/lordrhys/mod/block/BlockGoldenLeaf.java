...
import com.lordrhys.mod.world.ColorizeLeaves;

import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;

public class BlockGoldenLeaf extends BlockLeavesBase implements IShearable
{
    public static final String[] LEAF_TYPES = new String[] {"golden"};
    public static final String[][] Leaf_Icons = new String[][] {{"leaves_golden"}, {"leaves_golden_opaque"}};
    ...
    public int getRenderColor(int par1)
    {
    	if ((par1 & 3) == 0)
    	{
    		return ColorizeLeaves.getFoliageColorGolden();
    	}
//    	else if ((par1 & 3) == 0)
//    	{
//    		return ColorizeLeaves.getFoliageColorGolden();
//    	}
    	else return ColorizeLeaves.getFoliageColorGolden();
    }
    ...
    public int colorMultiplier(IBlockAccess par1IBlockAccess, int par2, int par3, int par4)
    {
        ...
        else
        {
            ...
            for (int l1 = -1; l1 <= 1; ++l1)
            {
                ...
                {
                    int j2 = par1IBlockAccess.getBiomeGenForCoords(par2 + i2, par4 + l1).getBiomeFoliageColor(229,193,0);
                    i1 += (j2 & 16711680) >> 16;
                    j1 += (j2 & 65280) >> 8;
                    k1 += j2 & 255;
                }
                ...
                if (world.checkChunksExist(x - i1, y - i1, z - i1, x + i1, y + i1, z + i1))
                {
                    ...
                    for (l1 = -b0; l1 <= b0; ++l1)
                    {
                        for (i2 = -b0; i2 <= b0; ++i2)
                        {
                            for (j2 = -b0; j2 <= b0; ++j2)
                            {
                            	block = world.getBlock(x + l1, y + i2, z + j2);
    ...
    {
        for (int i = 0; i < Leaf_Icons.length; ++i)
        {
            this.iconArray[i] = new IIcon[Leaf_Icons[i].length];

            for (int j = 0; j < Leaf_Icons[i].length; ++j)
            {
                this.iconArray[i][j] = par1IconRegister.registerIcon(LordRhysModMain.modid + ":" + Leaf_Icons[i][j]);
            }
        }
    }
